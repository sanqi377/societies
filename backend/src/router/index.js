  ...
  if (to.path != "/login") {
    ...
    if (store.state.token) {
      ...
      if (!store.state.menus) {
        // 判断是否有menu数据
        axios
          .post(setting.baseURL + "/admin/system/getMenu", {
            role: store.state.userinfo.role,
          })
          .then((res) => {
            // 后端请求menu数据, 带特定标记 route
            ...
            if (store.state.menus) {
              ...
              route.children.push({
                path: "/refresh",
                component: () => import("@/views/common/refresh.vue"),
              });
              route.children.push({
                path: "/societies/details/:id",
                component: () => import("@/views/societies/details"),
              });
              //增加404子路由
              // route.children.push(
              //   {
